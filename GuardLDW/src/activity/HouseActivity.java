...
public class HouseActivity extends BaseActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        userDB = new HeroDB(HouseActivity.this, "user.db");
        ...
        if(userList != null){
        	...
        	for(int i = 0; i < userList.size(); i++){
        		...
        		if(!userList.get(i).getComment().equals("")){
        			
        			commentList.add("玩家XXX:" + userList.get(i).getComment());
        ...
        adapter = new ArrayAdapter<String>(HouseActivity.this, android.R.layout.simple_list_item_1, commentList);
        commentListView.setAdapter(adapter);
	}
	...
	private class Listener implements View.OnClickListener{
		...
		public void onClick(View v) {
			...
			if(v.getId() == R.id.button_housegameback){
				...
				//假设每次按评论的用户都不同
				User currentUser = new User("14081206", "******", comment);
				
				//假设能够提交到服务器并收到正确的返回信息
				commentList.add("玩家XXX:" + comment);
				...
				HttpUtil.sendHttpPostRequest("http://GuardLDW/index.php", currentUser, new HttpCallBackListener(){
					...
					public void onFinish(String response) {
						...
						//如果评论失败
							...
							Toast.makeText(HouseActivity.this, "评论失败", Toast.LENGTH_SHORT);
						}
		...
		public boolean onItemLongClick(AdapterView<?> parent, View view, final int position, long id) {
			...
			dialog.setPositiveButton("确认", new DialogInterface.OnClickListener(){
				...
				public void onClick(DialogInterface dialog, int which){
					...
					//删除该用户评论,即设置该用户的评论为空
					userDB.deleteUserComment(position + 1);
